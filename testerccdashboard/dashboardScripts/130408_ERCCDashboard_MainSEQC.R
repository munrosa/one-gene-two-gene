...
# study = "SEQC_Main";siteName = "MAY";platform = "ILM";analysis = "NCTR";libeSizeNorm = T;printPDF = T; DEtest = T; totalSeqReads = T; sample1 = "A";sample2 = "B";  FCcode = data.frame(Ratio = c("a","b","c","d"),FC =  c(4,1,.667,.5)); myYLimMA = c(-3.5,3.5); myXLim = c(-10,15); choseFDR = 0.01; legendLabels = c("4:1","1:1","1:1.5","1:2"); ERCCdilution = 1; spikeVol = 50; totalRNAmass = 2.5*10^(3)
...
r_m.res = glm_est_r_m(site = filenameRoot, cnt = sampleLibeDataNoNorm, libeSizeNorm = libeSizeNorm,totalSeqReads = totalSeqReads,totalReads = totalReads, idCols = idCols, FCcode = FCcode,sample1 = sample1, sample2 = sample2,legendLabels=legendLabels,myXLim=myXLim,avexlabel = ERCCxlabelAve, printPlot = F)
...
print("Signal-Abundance Plots for dynamic range estimation...") 
  dynRangeDat = dynRangePlot(expressDat = sampleLibeDataNorm, designMat = designMatSum,idCols = idColsAdj, sampleNames = c(sample1,sample2),myXLim=myXLim,myYLim=NULL, legendLabels=legendLabels,FCcode=FCcode, indivxlabel = ERCCxlabelIndiv, avexlabel= ERCCxlabelAve)
