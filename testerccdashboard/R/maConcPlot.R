...
maConcPlot <-function(expDat, LODR.annot.ERCC, alphaPoint = 0.8,r_mAdjust = T, replicate = T, ReplicateName){
  ...
  #countPair$Mdat = log2(countPair[,3]) - log2(countPair[,4])
  #countPair$Adat = log2((countPair[,3] + countPair[,4])/2)
  ...
  if(replicate == T){
   # Use Average Concentration values (instead of Average Signals)
    FeatureA = data.frame(Feature= idCols$Feature, A = log2((idCols$Conc1 + idCols$Conc2)/2))
   for (i in 1:nlevels(FeatureA$Feature)){
     maData$A[which(maData$Feature == as.character(FeatureA$Feature[i]))] = FeatureA$A[i]  
   }
  ...
  # For the ERCCs that are plotted take the log ratio data and subtract the log nominal ratio, this will normalize the data
  ...
  print("Global Ratio SD for this sample pair is:")
  print(sdGlobal)
  ...
  sdRatioplot <- ggplot() + geom_point(data = ratVarDat, aes(x = A, y = M.SD, colour = Ratio), size = 5, alpha = alphaPoint)+ colScale
  ...
  # add fitted curve
  lineDat <- data.frame("xdat" = ratVarDat$A,"ydat" = predict(lmfit, x = ratVarDat$A)) 
  if(!is.null(expmod)) expDat <- data.frame("xdat" = ratVarDat$A,"ydat" = predict(expmod, list(x = ratVarDat$A))) 
  sdRatioplotFit <- sdRatioplot + geom_line(data = lineDat,aes(x=xdat,y = ydat)) + ylab("sd(Log2 Ratios of Counts)") + xlab(expression(paste("Log2 Average ERCC Abundance in Pure Spike Mixes (attomole nt/",mu,"L Mix)",sep = "")))
  
  if(!is.null(expmod)) sdRatioplotFit <- sdRatioplotFit + geom_line(data = expDat, aes(x = xdat, y = ydat))
  ...
  if (length(cutoffs)>0){
   ...
   for (i in 1:length(cutoffs)){
     maDataAveSD$LODR[which((maDataAveSD$A > cutoffs[i])&(maDataAveSD$Ratio == FCcodeC$Ratio[i]))] = "above"
   }
   
   maDataAveSD$LODR <- as.factor(maDataAveSD$LODR)
   rm_dat = data.frame("Mean r_m" = signif(r_m.mn,digits = 4), "Lower Limit" = signif(r_m.LL,digits = 4), "Upper Limit" = signif(r_m.UL,digits = 4))
   colnames(rm_dat) <- c("Mean", expression(paste("95% CI Lower Bound")), expression(paste("95% CI Upper Bound")))
   rownames(rm_dat) <- expression(r[m])
   
   #write.csv(maDataAveSD,file = paste(filenameRoot,"maDataAveSD.csv",sep = "."))
   # Plot ratio-signal data coding points below the LODR with open circles
   maPlot <- ggplot(maDataAveSD, aes(x = A, y = M.Ave) ) + geom_errorbar(aes(ymax = M.Ave + M.SD, ymin = M.Ave - M.SD,colour = Ratio),alpha = 0.3) + geom_point(aes(colour = Ratio),size = 5, alpha = alphaPoint) + geom_point(data = subset(maDataAveSD, (LODR == "below")),colour = "white",size = 2.5) + geom_hline(aes(yintercept = log2(Empirical), colour = Ratio), linetype = "longdash",alpha = 0.7) + ylab("Log2 Ratio of Counts") + xlabel + coord_cartesian(xlim = myXLim, ylim = myYLim) + colScale +annotation_custom(tableGrob(rm_dat,parse=T), xmin = 0.25*max(maDataAveSD$A), xmax = max(maDataAveSD$A), ymin = (myYLim[2]) - 0.25*myYLim[2],  ymax = myYLim[2] ) 
   }else{
    # Plot ratio-signal data without LODR coding
   maPlot <- ggplot(maDataAveSD, aes(x = A, y = M.Ave, colour = Ratio) ) + geom_errorbar(aes(ymax = M.Ave + M.SD, ymin = M.Ave - M.SD),alpha = 0.3) + geom_point(size = 5, alpha = alphaPoint) + geom_hline(aes(yintercept = log2(Empirical), colour = Ratio), linetype = "longdash",alpha = 0.7) + ylab("Log2 Ratio of Counts") + xlabel + colScale+ coord_cartesian(xlim = myXLim, ylim = myYLim)
  }
}else{
  ...
  print("Printing MA plot with LODR coding")
