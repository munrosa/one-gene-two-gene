testDE<- function(sampleInfo, expDat, cnt = cnt, info = info){
  ...
  sim.pval.res<-cbind(rowMeans(simcnt),use.res$P.values$QLSpline[-(1:nrow(cnt))],use.res$log.P.values$QLSpline[-(1:nrow(cnt))],use.res$F.stat$QLSpline[-(1:nrow(cnt))],
  rep(unique(ERCC.FC[!is.na(ERCC.FC[,2]),2]),each=49),rep(use.res$d0[2],nrow(simcnt)))
  colnames(sim.pval.res)<-c("MnCnt","Pval","LogPval","F.stat","Fold","Den.df");rownames(sim.pval.res)<-rownames(simcnt)
  ...
  pval.res<-cbind(rowMeans(cnt[ERCC,]),ERCC.pvals.adj,ERCC.log.pvals.adj,ERCC.F.stat.adj,ERCC.FC[ERCC,2],rep(use.res.adj$d0[2],length(ERCC.pvals.adj)))
  colnames(pval.res)<-c("MnCnt","Pval","LogPval","F.stat","Fold","Den.df")
