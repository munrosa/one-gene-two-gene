...
#                   totalSeqReads = T, libeSizeNorm = T, myYLimMA = c(-3.5,3.5),
#                   myXLim = c(-10,15),myYLim = NULL, filenameRoot = NULL)
countTable = COH.RatTox.ILM.NAP.CTL.countTable
totalReads = COH.RatTox.ILM.NAP.CTL.totalReads
designFactors = c("Sample","Library")
...
expDat <- initDat(countTable, totalReads, designFactors, sample1Name, 
...
maPlotAB = maConcPlot(expDat, LODR.annot.ERCC, alphaPoint = 0.8, r_mAdjust = T, 
                      replicate = T, ReplicateName="Library")
