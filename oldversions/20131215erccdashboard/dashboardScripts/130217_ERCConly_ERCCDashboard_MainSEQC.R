...
# siteName = "MAY";platform = "ILM";analysis = "NCTR";libeSizeNorm = T;printPDF = T; DEtest = T; totalSeqReads = F; sample1 = "A";sample2 = "B";  FCcode = data.frame(Ratio = c("a","b","c","d"),FC =  c(4,1,.667,.5)); myYLimMA = c(-3,3); myXLim = c(-7,18); choseFDR = 0.05; legendLabels = c("4:1","1:1","1:1.5","1:2")
...
### choose only the ERCCs for further analysis
# Transcripts = rbind(TranscriptsERCCOnly, TranscriptsHumanOnly)
Transcripts = TranscriptsERCCOnly
###############################################################################################################
