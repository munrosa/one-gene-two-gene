...
expression experiments. Two types of data from the SEQC project were 
analyzed.
\begin{enumerate}
...
file total reads are used. Please note that the current version of the package
automatically uses a library size normalization, therefore count data is 
expected. A future version of the package will be flexible to also accept data
...
\subsection{Use LODR estimates to Annotate MA Plot}
...
also save the figures to a pdf file.
\begin{center}
...
the 4 manuscript figures to a single page (plotsPerPg = ``''manuscript``''). If 
...
Laboratories}
If you wish, save your expDat list to an Rdata file that can be reused for
...
\subsection{Load data and define input parameters}
Note that due to file size limitations only a subset of the data shown in the
manuscript are provided with the package (data is from 6 out of 9 laboratories).
...
\subsection{Dashboard analysis of SEQC Lab 5 data}
The commands used in the Rat toxicogenomics sample can be repeated on the SEQC
data for Lab 5.
\begin{center}
<<estRMForLab5>>=
expDat <- est_r_m(expDat)
...
And if you wish, save your analysis results to an Rdata file that can be reused 
for comparisons across experiments or between laboratories.
<<saveResults>>=
save(expDat,file=paste0(expDat$sampleInfo$filenameRoot,".RData"))
@

\section{Alternative Spike-in Designs}
...
contol mixtures could be created with a ternary ratio design, three subpools of 
...
the package functions can be used. For initDat use ERCCMixes="Single" and  
\verb|est_r_m| and \verb|dynRangePlot| functions can then be used to examine the
mRNA fraction differences for the pair of samples and evaluate the dynamic range
of the experiment. 
...
manuscript and were obtained in R version 2.15.3. Slightly different results will
be obtained if R version 3.0.2 and associated packages are used due to 
differences in the differential expression testing packages.
Here is the session information for this analysis in R 2.15.3.
<<sessionData>>=
sessionInfo()
@
