...
# study = "SEQC_RatTox"; siteName = "COH";platform = "ILM";analysis = "RatTox";libeSizeNorm = T;printPDF = T; DEtest = T; totalSeqReads = T; sample1 = "NIT";sample2 = "CTL";  FCcode = data.frame(Ratio = c("a","b","c","d"),FC =  c(4,1,.667,.5)); myYLimMA = c(-3.5,3.5); myXLim = c(-10,15); choseFDR = 0.1; deliveryMet = "NN_IP"; legendLabels = c("4:1","1:1","1:1.5","1:2"); ERCCdilution = 1/100; spikeVol = 1; totalRNAmass = 0.500;
...
print("Check for sample mRNA fraction differences...")
  ...
  r_m.res = glm_est_r_m(site = filenameRoot, cnt = TranscriptsAB, libeSizeNorm = libeSizeNorm,totalSeqReads = totalSeqReads,totalReads = totalReads, idCols = idCols, FCcode = FCcode, sample1 = sample1,sample2=sample2, legendLabels = legendLabels,myXLim=myXLim, avexlabel = ERCCxlabelAve)
