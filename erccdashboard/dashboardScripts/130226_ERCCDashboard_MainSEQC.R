...
# siteName = "MAY";platform = "ILM";analysis = "NCTR";libeSizeNorm = T;printPDF = T; DEtest = T; totalSeqReads = T; sample1 = "A";sample2 = "B";  FCcode = data.frame(Ratio = c("a","b","c","d"),FC =  c(4,1,.667,.5)); myYLimMA = c(-3,3); myXLim = c(-7,18); choseFDR = 0.05; legendLabels = c("4:1","1:1","1:1.5","1:2"); ERCCdilution = 1; spikeVol = 50; totalRNAmass = 2.5*10^(6); ERCCxlabel  = "Log2 ERCC Spike Amount (attomol nt)"
...
# spikeVol = the amount of diluted ERCC mix spiked into the total RNA sample, units are in uL
# totalRNAmass = mass of total RNA spiked with the diluted ERCC mix, units are in ng
...
if ((libeSizeNorm == T)&(totalSeqReads == T)){
  ...
  ### ERCCdilution = 1; spikeVol = 50; totalRNAmass = 2.5*10^(6)
...
###############################################################################################################
  ...
  r_m.res = glm_est_r_m(site = filenameRoot, cnt = sampleLibeDataNoNorm, libeSizeNorm = libeSizeNorm,totalSeqReads = totalSeqReads,totalReads = totalReads, idCols = idCols, FCcode = FCcode,sample1 = sample1, sample2 = sample2,legendLabels=legendLabels,myXLim=myXLim,ERCCxlabel=ERCCxlabel)
...
### Evaluate dynamic range
  dynRangeDat = dynRangePlot(expressDat = sampleLibeDataNorm, designMat = designMatSum,idCols = idColsAdj, sampleNames = c(sample1,sample2),myXLim=myXLim,myYLim=NULL, legendLabels=legendLabels,FCcode=FCcode, ERCCxlabel = ERCCxlabel)
